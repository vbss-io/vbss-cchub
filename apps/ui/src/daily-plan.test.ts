...
test("carryOverFromYesterday keeps tasks not done, honoring explicit overrides over the raw checked state", () => {
  const tasks: DailyYesterdayTask[] = [
    ...
    { line: 6, depth: 0, checked: false, text: "Reopened by user", raw: "- [ ] Reopened by user", block: "b3" },
  ];
  const items = carryOverFromYesterday(tasks, { 6: true });
